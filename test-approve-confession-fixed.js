...
async function testApproveConfessionFixed() {
    try {
        console.log('🔍 Test Approve Confession Process (Fixed)...');
        ...
        // Tạo một confession test mới để test
        console.log(`\n📝 2. Tạo confession test mới:`);
        try {
            const confessionId = await db.addConfession(
                guildId,
                'test_user_approve',
                'Đây là confession test để kiểm tra quá trình approve sau khi fix',
                false
            );
            ...
            if (confessionId) {
                ...
                if (testConfession) {
                    ...
                    const approvedConfession = await db.updateConfessionStatus(
                        ...
                        'approved',
                        'test_admin_fixed',
                        'test_message_id_fixed',
                        'test_thread_id_fixed'
                    );
                    ...
                    if (approvedConfession) {
                        ...
                        console.log(`\n📊 4. Check guild settings after approve:`);
                        ...
                        console.log(`\n📊 5. Check if confession exists with new number:`);
                        ...
                        console.log(`\n📊 6. Check all recent confessions:`);
...
// Chạy test
testApproveConfessionFixed().then(() => {
